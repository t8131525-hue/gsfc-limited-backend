...
class ProductGrade(AuditableMixin, models.Model):
    ...
    class Meta:
        ...
        permissions = [
            ...
            ("can_manage_product_grades", "Can add, edit, delete product grade definitions"),
        ]
    def clean(self):
       
        # Rule: A product cannot have grades if it already has direct parameters.
        if self.product and ParameterDefinition.objects.filter(product=self.product, product_grade__isnull=True).exists():
...
class ParameterDefinition(AuditableMixin, models.Model):
    ...
    enum_options = models.JSONField(blank=True, null=True, help_text="For ENUM type, provide options as a JSON array, e.g., [\"Option1\", \"Option2\"]")
    min_value = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True, help_text="The minimum normal value for this parameter.")
    max_value = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True, help_text="The maximum normal value for this parameter.")
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='parameters', null=True, blank=True)
    product_grade = models.ForeignKey(ProductGrade, on_delete=models.CASCADE, related_name='parameters', null=True, blank=True)
    boolean_true_label = models.CharField(max_length=50, blank=True, null=True, help_text="Custom label for the 'true' value, e.g., 'Present', 'Yes'.")
    boolean_false_label = models.CharField(max_length=50, blank=True, null=True, help_text="Custom label for the 'false' value, e.g., 'Absent', 'No'.")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        permissions = [
            ...
            ("can_manage_parameter_definitions", "Can add, edit, delete parameter definitions"),
        ]
    ...
    def clean(self):
        ...
        if self.product and self.product_grade:
            raise ValidationError(
                _("A parameter definition cannot be associated with both a Product and a Product Grade. Select EITHER one or the other."),
                code='invalid_scope'
            )
        if not self.product and not self.product_grade:
            raise ValidationError(
                _("A parameter definition must be associated with either a Product or a Product Grade."),
                code='missing_scope'
            )
        ...
        if self.data_type == 'BOOLEAN':
            if not self.boolean_true_label or not self.boolean_false_label:
                raise ValidationError(
                    _("For BOOLEAN data type, both 'True Label' and 'False Label' are required."),
        ...
        if self.product:
            if queryset.filter(name=self.name, product=self.product, product_grade__isnull=True).exists():
                raise ValidationError(
                    _("A parameter with this name already exists for this product."),
                    code='duplicate_product_parameter'
                )
        elif self.product_grade:
            if queryset.filter(name=self.name, product_grade=self.product_grade).exists():
                raise ValidationError(
                    _("A parameter with this name already exists for this product grade."),
...
class TestRecord(AuditableMixin, models.Model):
    ...
    record_id = models.CharField(max_length=20, unique=True, editable=False, null=True, blank=True, help_text="Auto-generated unique test record ID, e.g. TR20250731-01")
    
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='test_records')
    product_grade = models.ForeignKey(ProductGrade, on_delete=models.SET_NULL, null=True, blank=True, related_name='test_records')
    batch_no = models.CharField(max_length=255, help_text="The batch number for the sample, e.g., 'A'")
    sample_id = models.CharField(max_length=255, help_text="Identifier for the sample tested. Not unique, as retests will share this ID.")
    test_date = models.DateField(auto_now_add=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PENDING')

    analyst = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='performed_tests')
    ...
    def clean(self):
        ...
        if self.product:
            ...
            if has_grades and not self.product_grade:
                raise ValidationError(
                    _("This product has grades; a specific product grade must be selected for the test record."),
                    code='product_grade_required'
                )
            if not has_grades and self.product_grade:
                raise ValidationError(
                    _("This product does not have grades; a product grade cannot be selected for the test record."),
            ...
            if self.product_grade and self.product_grade.product != self.product:
                 raise ValidationError(
                    _("The selected product grade does not belong to the specified product."),
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new and not self.record_id:
            ...
            last_record_for_today = TestRecord.objects.filter(
                created_at__date=self.created_at.date()
            ).exclude(pk=self.pk).order_by('-record_id').first()
            ...
            if last_record_for_today and last_record_for_today.record_id:
                try:
                    ...
                    sequence = TestRecord.objects.filter(created_at__date=self.created_at.date()).count()
    ...
    def __str__(self):
        ...
        retest_str = f" (Retest of {self.retest_of.record_id})" if self.retest_of else ""
        return f"{base}{grade_str} ({self.sample_id}){retest_str}"


class TestResult(AuditableMixin, models.Model):
    test_record = models.ForeignKey(TestRecord, on_delete=models.CASCADE, related_name='parameter_values')
    parameter = models.ForeignKey(ParameterDefinition, on_delete=models.CASCADE, related_name='test_values')
    # Store values based on data_type
    value_decimal = models.DecimalField(max_digits=10, decimal_places=4, blank=True, null=True)
    ...
    class Meta:
        unique_together = ('test_record', 'parameter') # A parameter can only have one value per test record
    ...
    def clean(self):
        ...
        if param_def.is_required and num_values_provided == 0:
            raise ValidationError(f"A value is required for the required parameter '{param_def.name}'.")
        
        if num_values_provided > 1:
            raise ValidationError("Only one value field (decimal, string, or boolean) should be populated.")

        if has_decimal and param_def.data_type not in ['INTEGER', 'DECIMAL']:
            raise ValidationError(f"A numeric value is not allowed for the '{param_def.name}' parameter (type: {param_def.data_type}).")
        
        if has_boolean and param_def.data_type != 'BOOLEAN':
            raise ValidationError(f"A boolean value is not allowed for the '{param_def.name}' parameter (type: {param_def.data_type}).")
            
        if has_string and param_def.data_type not in ['STRING', 'ENUM']:
             raise ValidationError(f"A string value is not allowed for the '{param_def.name}' parameter (type: {param_def.data_type}).")
