...
from .serializers import (
    ProductSerializer, ProductGradeSerializer, ParameterDefinitionSerializer,
    TestRecordSerializer, TestResultInputSerializer,  TestResultDisplaySerializer, AssignAnalystSerializer
)
...
class TestRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        is_manager_or_supervisor = user.groups.filter(name__in=['Manager', 'Supervisor', 'Admin']).exists()
        is_analyst_only = user.groups.filter(name='Analyst').exists() and not is_manager_or_supervisor
    ...
    def assign(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            return Response(self.get_serializer(test_record).data, status=status.HTTP_200_OK)
        else:
    ...
    def order_retest(self, request, pk=None):
        ...
        user = request.user
        if not user.has_perm('inventory.can_approve_test_records'):
            return Response({"detail": "You do not have permission to order a retest."}, status=status.HTTP_403_FORBIDDEN)
    ...
    def approve_reject(self, request, pk=None):
        ...
        comments = request.data.get('supervisor_comments', test_record.supervisor_comments)
        ...
        old_approval_user = str(test_record.approved_by) if test_record.approved_by else ''
        old_comments = test_record.supervisor_comments if test_record.supervisor_comments else ''
        ...
        details = {
            ...
            'supervisor_comments': [old_comments, comments if comments is not None else ''],
            'approved_by': [old_approval_user, str(user)]
        }
        log_custom_event(instance=test_record, action_type=new_status, details=details, user=user)
...
# ... (keep all your existing ViewSets) ...
